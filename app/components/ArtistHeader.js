...
const ArtistHeader = ({ artist }) => {
    ...
    // Maintain the same header style as in your main header
    const headerStyle = {
        background: 'linear-gradient(to bottom, #333, #000)', // Consistent black gradient
        height: '20vh' // Increase the height to accommodate the artist name at the bottom
    };

    return (
        <>
            <header className="fixed top-0 left-0 w-full px-4 z-10" style={headerStyle}>
                ...
                {isArtistPage && artist && (
                    <div className=" -mt-28 w-full flex justify-center items-end pb-4">
                        <h1 className="text-6xl md:text-7xl lg:text-8xl xl:text-9xl text-white font-bold">{artist.name}</h1>
                    </div>
                )}
            </header>
        </>
