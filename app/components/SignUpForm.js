        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex justify-center items-center p-4">
            <div className="bg-green-500 p-8 rounded-2xl w-full max-w-4xl">
                    ...
                    <div className="flex justify-center space-x-2">
                        {['artist', 'manager', 'promoter'].map((item) => (
                            <button
                                ...
                                style={errors.role ? { borderColor: 'red', borderWidth: 5 } : {}}
                            >
                                {item.charAt(0).toUpperCase() + item.slice(1)}
                            </button>
                        ))}
                    </div>
                    {errors.role && <p className="text-2xl text-red-600 -mt-14">{errors.role}</p>}
                </div>
                <form onSubmit={handleSubmit} className="text-2xl mt-6 space-y-4">
                    ...
                    {errors.spotifyLink && <p className="text-red-600 text-sm">{errors.spotifyLink}</p>}
                    ...
                    {message && <div className="fixed inset-0 bg-black bg-opacity-50 flex justify-center items-center text-5xl text-white z-50">
